...
namespace HEC.FDA.Model.alternatives
{
    public class Alternative : ValidationErrorLogger, IProgressReport
    {
        ...
        private static bool CanCompute(int baseYear, int futureYear, int periodOfAnalysis)
        {
            ...
            {
                canCompute = false;
                MessageEventArgs messageArguments = new(new Message($"Cannot complete calculation. Evaluate the base year {baseyear} is larger thant the future year {futureYear}.Please resubmit."));
            }
            int differenceBetweenBaseAndFutureYearInclusive = futureYear - baseYear + 1;
            if (differenceBetweenBaseAndFutureYearInclusive < 2)
            {
                canCompute = false;
                MessageEventArgs messageArguments = new(new Message($"The gap between base year and future year must be larger than 2. Please resubmit."));
            }
            if (differenceBetweenBaseAndFutureYearInclusive > periodOfAnalysis)
            {
                canCompute = false;
                MessageEventArgs messageArguments = new(new Message($"The period of analysis is too large. Ensure the period of analysis is {periodOfAnalysis} or less and resubmit."));
            }
