...
//brennan: This is a cool way to return multiple values, but I don't think it's necessary here. We can get min from the property getter, and we could make a really simple get valueStep method.
//I'd like to refactor this to only return cumulative frequencies as the method name implies.

//problem is this returns three types, three different values and the method validates the single responsibility principlal. this method now has 3 responsibilities. must refactor and simplify down to one type.
//one cumulative probability returned
//figure out how to ge the min and the value step separate from this method.  - value step= bin width. creating a histogram = value step is the width of this histogram. 
//these are results of the compute
//we use histograms to collect the results , we calculate a damaged value 200,000 time and then collect in historgram
//this case is emprical distribution ... the empircal distribution might only have eight coordinates so we may only know the eight values for eight probabilities.  nice histogram based on eight point and interpolating.
//empircal cumulative distribution - a distribution that holds their values and cumulative probabilities. in computational statistics we work with non parametric distributions... without distributional assumptions.
//not being used to collect, is being used as an input to speicfiy a distribution. like distribution of flows. what is the ditribution of probable flows in the river per given year.
//use empriical distribution to reflect the flows because when there are anthropogenic factors on rives rivers stop behaving like they typically do. the behavior becomes very messy. when humans haven t doen anything to a river we can model what that river will do but anthropogencis influences and there is no analytical solution.
//specifying the distribution of flow that is going to be sampled in the compute.
